export const ImprintSection = () => (
  <div className="mt-4 flex flex-col gap-y-4 font-nice-regular text-lg md:text-xl lg:mt-12 lg:gap-y-12 lg:text-2xl xl:text-3xl">
    <div>
      <p className="font-nice-bold">RESPONSIBLE FOR THE CONTENT</p>
      <p>
        <br />
        Solomiya Collective NGO
        <br />
        29, Vyshneva street, Tatsenky village, Obukhiv raion, Kyivs&apos;ka oblast
        <br /> 08700 Ukraine
        <br />
        hello@solomiyamag.com
        <br />
      </p>
    </div>
    <div>
      <p className="font-nice-bold">WEB DESIGN</p>
      <p>
        <br />
        Mélan Rouillon, Peter B&uuml;nnagel (Kollektiv Scrollan)
        <br />
      </p>
    </div>
    <div>
      <p className="font-nice-bold">WEB DEVELOPMENT</p>
      <p>
        <br />
        Ivan Hrytsenko
        <br />
      </p>
    </div>
    <div>
      <p className="font-nice-bold">DONATIONS WITHIN UKRAINE</p>
      <br />
      <p>
        Отримувач: Громадська організація «СОЛОМІЯ КОЛЛЕКТІВ»
        <br />
        ЄДРПОУ 45256869
        <br />
        Цільові IBAN рахунки для ГО СОЛОМІЯ КОЛЛЕКТІВ:
        <br />
        UA803052990000026004036230359 — UAH, UA773052990000026002046233412 — EUR, UA433052990000026007006236874 — USD
        <br />
        АТ КБ «ПРИВАТБАНК»
        <br />
        Призначення платежу: «Безповоротна фінансова допомога на здійснення статутної діяльності ГО». Здійснюючи внесок,
        ви погоджуєтесь із умовами&nbsp;
        <p className="inline">
          <a
            href={`https://www.solomiyamag.com/ngo_solomiya_collective.pdf`}
            className="italic transition-all hover:underline"
            target="_blank"
            rel="noopener noreferrer"
          >
            Публічної оферти ГО “СОЛОМІЯ КОЛЛЕКТІВ”.
          </a>
        </p>{' '}
      </p>
    </div>
    <div>
      <p className="font-nice-bold">DONATIONS FROM ABROAD</p>
      <br />
      <p>
        By donating, you agree to the&nbsp;
        <p className="inline">
          <a
            href={`https://www.solomiyamag.com/ngo_solomiya_collective.pdf`}
            className="italic transition-all hover:underline"
            target="_blank"
            rel="noopener noreferrer"
          >
            Public Offer of the “SOLOMIYA COLLECTIVE” NGO.
          </a>
        </p>{' '}
        <br />
        Name of the recipient: NGO SOLOMIYA COLLECTIVE / ГО СОЛОМІЯ КОЛЛЕКТІВ / GO SOLOMIIA KOLLEKTIV
        <br />
        Name of the bank: JSC CB ”PRIVATBANK”, 1D HRUSHEVSKOHO STR., KYIV, 01001, UKRAINE
        <br />
        Bank SWIFT code: PBANUA2X
        <br />
        Transfer description: Donation for NGO SOLOMIYA COLLECTIVE
        <br />
        <br />
        <span className="underline">Donations in Euro</span>
        <br />
        IBAN code - EUR:
        <br />
        UA773052990000026002046233412
        <br />
        Account in correspondent bank: 400886700401
        <br />
        SWIFT code of the correspondent bank: COBADEFF
        <br />
        Correspondent bank: Commerzbank AG, Frankfurt am Main, Germany
        <br />
        or
        <br />
        Account in correspondent bank: 6231605145
        <br />
        SWIFT code of the correspondent bank: CHASDEFX
        <br />
        Correspondent bank: J.P.MORGAN AG, FRANKFURT AM MAIN, GERMANY
        <br />
        <br />
        <span className="underline">Donations in US-Dollar</span>
        <br />
        IBAN code - USD: UA433052990000026007006236874
        <br />
        Account in correspondent bank: 001-1-000080
        <br />
        SWIFT code of the correspondent bank: CHASUS33
        <br />
        Correspondent bank: JP Morgan Chase Bank, New York, USA
        <br />
        or
        <br />
        Account in correspondent bank: 890-0085-754
        <br />
        SWIFT code of the correspondent bank: IRVT US 3N
        <br />
        Correspondent bank: The Bank of New York Mellon, New York, USA
        <br />
        or
        <br />
        Account in correspondent bank: 36445343
        <br />
        SWIFT code of the correspondent bank: CITI US 33
        <br />
        Correspondent bank: Citibank N.A., NEW YORK, USA
      </p>
    </div>
  </div>
);
