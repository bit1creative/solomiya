  ...
  return (
    <Dialog.Root open={isMenuOpen} onOpenChange={() => setIsMenuOpen(!isMenuOpen)}>
      <div className="h-screen overflow-hidden bg-main-issue-1" onClick={handleFrameClick}>
        <Dialog.Trigger>
          <img className="absolute left-2 top-2 z-30 h-14 w-12" src={logo} alt="logo" />
        </Dialog.Trigger>
      ...
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-40 h-screen bg-white" />
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 h-[calc(100%-40px)] w-[calc(100%-40px)] -translate-x-1/2 -translate-y-1/2 bg-main-issue-1">
